...
+(instancetype)charView
{
    ...
    // 默认值
    chartView.frame = CGRectMake(10, 150, 300 ,220);
    return chartView;
}
...
-(void)drawFuncLine
{
    if (self.xValues.count != 0 && self.yValues.count != 0) {
        ...
        NSMutableArray *funcPoints1 = [NSMutableArray array];
        ...
        if (self.xValues.count != self.yLowValues.count) {
            pointCount = (self.xValues.count < self.yLowValues.count ? self.xValues.count : self.yLowValues.count);
        }
        ...
        for (int i = 0; i < pointCount; i++) {
            ...
            CGFloat yValue = [self.yLowValues[i] floatValue];
            CGFloat funcYPoint1 = (yCoordinateHeight) - (yValue / self.maxYValue) * (yCoordinateHeight) + 3;
            [funcPoints1 addObject:[NSValue valueWithCGPoint:CGPointMake(funcXPoint, funcYPoint1)]];
        }
        ...
        UIBezierPath *funcLinePath1 = [UIBezierPath bezierPath];
        [funcLinePath1 moveToPoint:[[funcPoints1 firstObject] CGPointValue]];
        [funcLinePath1 setLineCapStyle:kCGLineCapRound];
        [funcLinePath1 setLineJoinStyle:kCGLineJoinRound];
        int index1 = 0;
        for (NSValue *pointValue in funcPoints1) {
            if (index1 != 0) {
                [funcLinePath1 addLineToPoint:[pointValue CGPointValue]];
                [funcLinePath1 moveToPoint:[pointValue CGPointValue]];
                [funcLinePath1 stroke];
            }
            index1++;
        }
        
        CAShapeLayer *lineLayer1 = [self setUpLineLayer];
        lineLayer1.path = funcLinePath1.CGPath;
        lineLayer1.strokeColor = self.lowChartColor.CGColor;
        CABasicAnimation *pathAnimation1 = [CABasicAnimation animationWithKeyPath:@"strokeEnd"];
        pathAnimation1.duration = 1.5;
        ...
        [lineLayer1 addAnimation:pathAnimation1 forKey:@"lineLayerAnimation"];
        lineLayer1.strokeEnd = 1.0;
